...
public interface AccountRepository extends JpaRepository<Account, Long> {
    ...
    List<Account> fetchAdminAccountsJPQL();


    //Write a JPQL query to sort all accounts with age
    @Query("SELECT a FROM Account a ORDER BY a.age DESC")
    List<Account>orderOrderByAgeJPQL();
    ...
    List<Account>retrieveAllAgeLessThanSQL(@Param("age") int age);
    ...
    List<Account>retrieveAllByContainsInAddressOrCountryOrStateContainsSQL(@Param("pattern") String pattern);

    //Write a native query to read all accounts with an age higher than a specific value
    @Query(value = "SELECT * FROM account_details WHERE age > ?1", nativeQuery = true)
    List<Account>retrieveHigherThanAgeSQL(int age);
